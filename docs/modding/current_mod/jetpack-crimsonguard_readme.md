...
`haven-city-chaos` declares a `*mod-chaos-jetpack-hook*` function pointer for exactly this. After
the merge, the chaos branch's `Debug ▸ Mods ▸ haven-city-chaos ▸ guards ▸ Jetpack guards` toggle
drives `mod-jetpack-tick`, and the two `traffic-manager::update` hook lines collapse into one.

The only textual merge conflict is those adjacent lines in `traffic-manager::update`. Resolve by
keeping both calls, or by routing the jetpack one through the chaos hook.
