...
- **"Mods" debug menu tab (City Peaceful & City Insurrection):** the debug menu includes a dedicated "Mods" tab:
  ...
  - `City Insurrection` (**Fully Implemented**): a three-front territorial civil war across Haven City, with districts classified by loaded city-level name (never hardcoded coordinates). The ambient city-guard pool is populated with whichever faction owns the district Jak is in, so districts are strictly territorial with no wasted spawn slots. **Slums** (`ctysluma`/`b`/`c`) — Blue rebel stronghold: strictly 100% lone blue guards with random weapons, and a police safe haven (any wanted level snaps to 0 on entry and cannot rise; hitting a blue guard triggers only its personal self-defense). **Loyalist districts** (everything else — Port, Bazaar, Farms, Palace, Stadium, downtown) — strictly 100% stock red/yellow guards, with fully vanilla police density and behaviour toward Jak. **Industrial** (`ctyinda`/`b`) — War Zone: 50/50 blue vs red at heavy density (~14 guards, the slots freed by removing all civilians and vehicles), and the two factions hunt and open fire on each other on sight (bursts, grenades, taser charges) — a living background battle that never raises Jak's wanted level. Switching modes in the menu, or crossing a district border, flushes and respawns the city guards so the rules apply immediately.
...
- **Onglet menu debug « Mods » (City Peaceful & City Insurrection) :** le menu debug comprend un onglet « Mods » :
  ...
  - `City Insurrection` (**Entièrement Implémenté**) : une guerre civile territoriale à trois fronts dans Haven City, les quartiers étant classés par le nom du niveau de ville chargé (jamais de coordonnées codées en dur). Le pool de gardes ambiants de la ville est peuplé de la faction qui contrôle le quartier où se trouve Jak, donc les quartiers sont strictement territoriaux sans aucun slot de spawn gaspillé. **Slums** (`ctysluma`/`b`/`c`) — bastion des rebelles bleus : strictement 100% de gardes bleus solitaires à arsenal aléatoire, et zone refuge de police (tout niveau de recherche retombe à 0 à l'entrée et ne peut plus monter ; frapper un garde bleu ne déclenche que son autodéfense personnelle). **Quartiers loyalistes** (tout le reste — Port, Bazar, Fermes, Palais, Stade, beaux quartiers) — strictement 100% de gardes rouges/jaunes classiques, avec une densité et un comportement de police strictement d'origine envers Jak. **Industriel** (`ctyinda`/`b`) — Zone de Guerre : 50/50 bleus vs rouges à forte densité (~14 gardes, les slots libérés en retirant tous les civils et véhicules), et les deux factions se repèrent et ouvrent le feu mutuellement à vue (rafales, grenades, charges au taser) — une bataille de fond vivante qui ne fait jamais monter le niveau de recherche de Jak. Basculer le mode dans le menu, ou franchir une frontière de quartier, vide et régénère les gardes pour appliquer les règles immédiatement.
