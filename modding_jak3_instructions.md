# AI Agent Modding Directive (Jak 3)
In addition to the AGENTS.md file, follow these instructions in case of modding Jak 3.

## 1. Context and Role
You are an expert developer agent assigned to modding the game Jak 3 via the OpenGOAL project. 
The entirety of the original trilogy was coded at over 98% in GOAL, a custom LISP language developed by Naughty Dog. Your goal is to create, modify, and test scripts and assets for the game Jak 3, while strictly respecting the existing architecture.
---
...
* **Mandatory Traceability (modding_jak3_changes.md):** You must imperatively maintain a `modding_jak3_changes.md` file at the root of the workspace. Every modification must be logged there.
  * **Required format in `modding_jak3_changes.md`:** Date | Touched/created files | Technical description of the modification | Objective of the mod.

---

## 3. Knowledge Base and OpenGOAL Architecture
* **GOAL Language:** The code uses a LISP syntax (parentheses, lists, functions). Always draw inspiration from the existing files in the `goal_src/jak3/` folder to understand the exact syntax.
...
* **Texture Replacement:** To modify textures, the new `.png` images must be placed respecting the strict directory structure in the `custom_assets/jak3/texture_replacements/` folder. 
* **Sources documentation:** : 
    - https://opengoal.dev/docs/
    - https://opengoal.dev/docs/source-docs/jak3/package-index
...
3. **Game Launch:** Execute the game with the specific command for Jak 2, for example `task boot-game` or `./gk.exe --game jak3`. 
