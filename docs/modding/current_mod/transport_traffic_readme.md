...
- **While deploying:**
  - **Base flight level maintained.** `*transport-v-deploy-descent*` is 0 m, so the ship stays in its normal traffic lane instead of lowering into bridges, streetlamps and buildings. Guards fall straight down via gravity (`behavior` 6).
...
- **Once the load is delivered** (`guards-dropped` reaches the cap) `transport-v-deploy-active?` goes false: spin damping releases, the hatch closes, and the stock guard chase + `vehicle-guard-method-153` turret fire take over — it hunts Jak like a hellcat with a gun.
...
- **Pendant la dépose :**
  - **Maintien du niveau de vol.** `*transport-v-deploy-descent*` est à 0 m, donc le vaisseau reste dans sa voie aérienne normale au lieu de descendre vers les lampadaires, ponts et bâtiments. Les gardes tombent directement au sol par gravité (`behavior` 6).
...
- **Une fois la charge livrée** (`guards-dropped` atteint le plafond) `transport-v-deploy-active?` devient faux : l'amortissement se relâche, la porte se ferme, et la chasse standard + le tir de tourelle `vehicle-guard-method-153` prennent le relais — il chasse Jak comme un hellcat armé.
