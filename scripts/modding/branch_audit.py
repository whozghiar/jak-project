...
BASE = "origin/master-dev"   # overridden to "master-dev" by --local
LOCAL = False                # --local: audit local branch heads vs local master-dev
...
def branch_ref(branch):
    """--local: always use the local head. Otherwise prefer origin/<branch>,
    falling back to the local ref for unpushed branches."""
    if LOCAL:
        return branch, True
...
def render(rows):
    ...
    for r in rows:
        md.append("| `{branch}`{lo} | {game} | {old} | {menu} | {tog} | {eng} | {rme} |".format(
            branch=r["branch"], lo=(" 🟡" if r["local_only"] and not LOCAL else ""), game=r["game"],
...
def main():
    ...
    ap.add_argument("--local", action="store_true",
                    help="Audit local branch heads against local master-dev "
                         "(use before pushing the harmonization commits).")
    args = ap.parse_args()

    if args.local:
        BASE, LOCAL = "master-dev", True
