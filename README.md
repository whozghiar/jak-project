...
- **"Mods" debug menu tab (City Peaceful & City Insurrection):** the debug menu includes a dedicated "Mods" tab:
  ...
  - `City Insurrection` (**Work in Progress / WIP**): territorial faction warfare across Haven City districts. Slums (`ctysluma`/`b`) designed as a Blue rebel stronghold (100% blue guards roaming individually with random weapons, safe haven where city alerts clear). Loyalist districts spawn 100% red guards with standard policing. The Industrial sector (`ctyinda`/`b`) serves as a Conflict Zone (50/50 spawns) where red and blue guards fight autonomously. (Spawning distribution and autonomous hostility currently in active tuning).
...
- **Onglet menu debug « Mods » (City Peaceful & City Insurrection) :** le menu debug comprend un onglet « Mods » :
  ...
  - `City Insurrection` (**En cours de développement / WIP**) : guerre de factions territoriale à travers les quartiers de Haven City. Les Slums (`ctysluma`/`b`) sont prévus comme le bastion des rebelles bleus (100% gardes bleus se déplaçant individuellement avec arsenal aléatoire, zone refuge). Les quartiers loyalistes génèrent 100% de gardes rouges. Le Secteur Industriel (`ctyinda`/`b`) sert de Zone de Conflit (génération 50/50) avec affrontements inter-factions autonomes. (Distribution des spawns et hostilité mutuelle en cours d'ajustement).
