...
>
> - **Applies to / Concerne :** Jak 2 (OpenGOAL PC Port)
...
> ### 📑 Summary / Sommaire
>
> - 🇬🇧 **English:** [A. Vocabulary](#a--vocabulary) · [B. Verified Blocks (B1–B11)](#b--verified-instruction-blocks) · [C. Jak 2 Subsystems (C1–C5)](#c--game-specific-verified-subsystems) · [D. Pitfalls & Traps (D1–D4)](#d--lisp-level-known-pitfalls) · [E. How to Contribute](#how-to-contribute)
> - 🇫🇷 **Français :** [A. Vocabulaire](#a--vocabulaire) · [B. Blocs Vérifiés (B1–B11)](#b--blocs-dinstructions-vérifiés) · [C. Sous-Systèmes Jak 2 (C1–C5)](#c--sous-systèmes-vérifiés-propres-à-jak-2) · [D. Pièges & Risques (D1–D4)](#d--pièges-connus-au-niveau-lisp) · [E. Comment Contribuer](#comment-contribuer)
...
## What this file is
The **source of truth** for how to write OpenGOAL (GOAL) Lisp for Jak 2. Every instruction here is **verified** — it has been compiled and seen working in game. Consult it *before* writing or changing any `.gc` file so you never invent an instruction that does not exist. Each entry says, in plain words, what a block does, gives one commented example, and lists the traps.

**Also read:** [`engine_generic_concepts.md`](engine_generic_concepts.md) (memory, heaps, DGOs, process life cycle).
...
| **`process-drawable`** | A process that also has a 3D model + transform (`engine/process-drawable/process-drawable.gc`). |
...
## B — Verified instruction blocks

### B1 — Define a custom actor type
...
### B3 — Drive animation: the `ja` macros
...
### B5 — Bind a model to a process: `initialize-skeleton`
...
### B8 — Custom art-groups & dynamic animation linking (`link-art!`)
...
### B10 — Generic enemy death effect (`do-effect`
...
### B11 — Register an in-game Mods toggle
...
## C — Game-specific verified subsystems

### C1 — Vehicles: flags, grab rails, driver methods
...
## E — ➕ Append new verified entries below
...
## Rôle de ce fichier
La **source de vérité** pour écrire du Lisp OpenGOAL (GOAL) pour Jak 2. Chaque instruction ici est **vérifiée** — elle a été compilée et vue fonctionner en jeu. À consulter *avant* d'écrire ou de modifier un fichier `.gc`, pour ne jamais inventer une instruction qui n'existe pas. Chaque entrée explique, en mots simples, ce que fait un bloc, donne un exemple commenté, et liste les pièges.

**À lire aussi :** [`engine_generic_concepts.md`](engine_generic_concepts.md) (mémoire, heaps, DGO, cycle de vie).
...
| Terme | Signification |
|---|---|
| **GOAL** | Le dialecte Lisp de Naughty Dog, compilé en x86-64 natif par OpenGOAL. Toute la logique du jeu (`goal_src/**/*.gc`). |
| **`goalc`** | Le compilateur OpenGOAL. REPL (`task repl`) ou batch (`-c "(...)"`). |
| **`gk`** | Le runtime C++ (« game kernel ») qui exécute le code compilé. |
| **`(mi)`** | Commande REPL : compilation incrémentale + rechargement à chaud du jeu. |
| **process** | Tâche coopérative légère avec sa propre pile/son propre heap. |
| **`process-drawable`** | Un processus qui a aussi un modèle 3D + une transformation. |
| **`*target*`** | Le symbole global qui pointe sur le processus joueur (Jak). |
| **`self` / `pp`** | Dans un `behavior` : `self` = le processus ; `(with-pp ...)` lie `pp` = le pointeur de processus. |
| **state** | Un nœud nommé de la machine à états d'un processus : `:event`, `:code`, `:post`. |
| **`ja`** | Famille de macros qui pilotent l'animation squelettique (« joint animation »). |
| **art-group** | Un paquet nommé : squelette + géométrie + animations d'un modèle. |
| **DGO** | Paquet disque d'objets compilés chargé d'un bloc. |

---

## B — Blocs d'instructions vérifiés

### B1 — Définir un type d'acteur custom
...
### B3 — Piloter l'animation : les macros `ja`
...
### B5 — Lier un modèle à un processus
...
### B8 — Art-groups custom & liaison dynamique
...
### B10 — `death-default`) / Effet de mort générique
...
### B11 — Enregistrer une bascule Mods en jeu
...
## C — Sous-systèmes vérifiés propres à Jak 2

### C1 — Véhicules : drapeaux, barres d'accroche, méthodes de conduite
...
### C3 — method residency / Résidence des états et méthodes virtuels
...
### C4 — Reskin à chaud d'un processus
...
### C5 — FR3 residency / Géométrie merc & résidence FR3
...
### D1 — Ordre des symboles et du chargement
...
### D3 — Une compilation propre n'est pas une validation
...
## E — Ajouter les nouvelles entrées vérifiées ci-dessous
