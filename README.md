...
> 📁 [`docs/modding/current_mod/jak2_skin_secret.mp4`](docs/modding/current_mod/jak2_skin_secret.mp4)  
...
Registers `(game-secrets jak-is-jak2)` into `*menu-secrets-array*` in `secrets-menu.gc` and provides text override in `progress-draw-pc.gc`.
...
Enregistre `(game-secrets jak-is-jak2)` dans `*menu-secrets-array*` dans `secrets-menu.gc` et configure l'affichage dans `progress-draw-pc.gc`.
